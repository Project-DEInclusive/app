...
const Component = ({ event, onClick }) => {
    ...
    const IsOver = () => {
        ...
        return new Date(event.time.endtime).getTime() < new Date().getTime();
    };
    const IsLive = () => {
        if (!event.time) return false;
        return new Date(event.time.starttime).getTime() > new Date().getTime() && IsOver();
    };
    return (
        <>
            <div
                className={`border cursor-pointer rounded-lg  shadow-lg hover:border-indigo-700 my-4 ${IsLive() == true ? "border-red-700" : "border-gray-300"}`}
                onClick={onClick}
            >
                ...
                <div className="px-5 py-3">
                    <h3 className="text-gray-800 font-semibold text-xl hover:underline">{event?.name}</h3>
                    ...
                    <div className="inline-flex text-gray-700 font-medium">
                        ...
                        <p className="font-normal text-sm text-gray-500  hover:underline">
                            {event?.place?.city}, {event?.place?.state}, {event?.place?.country}
                        </p>
                    </div>
                    <div className="flex flex-row gap-x-3 gap-y-2 flex-wrap mt-1">
                        <div className="bg-gray-200 py-1 px-2 rounded-md text-sm">
                            <span className="font-bold text-gray-700 inline-flex align-middle">
                                <ClockIcon className="w-5 h-5 mr-2 " /> {formatAMPM(event?.time?.starttime)}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex align-middle justify-between gap-x-3 mt-4">
                        <p className="text-gray-500 text-xs">posted {getDateDiffString(event?.createdAt)}</p>
                        <div className="flex  px-1 items-center -space-x-1 overflow-hidden">
                            {event?.speakers.slice(0, 3).map((speaker, index) => (
                                <img
                                    layout="fill"
                                    key={index}
                                    className="inline-block object-center object-cover h-8 w-8 rounded-full ring-2 ring-white"
                                    src={`${process.env.NEXT_PUBLIC_IMAGE_URL}${speaker?.name}?key=${speaker?.thumbnail?.src}`}
                                    alt={speaker?.name}
                                />
                            ))}
                        </div>
