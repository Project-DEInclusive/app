...
// Set create permissions for the 'companymember' collection for the 'Pro' role
const createPermission = new CActionPermission("create");
createPermission.allowed = true;
createPermission.immutableprops = ["_id", "ownerapproval", "adminapproval", "draft", "deleted"]; // Define the properties that can't be updated by 'Pro' role
createPermission.hiddenprops = ["__v", "company.owner"]; // Define the properties dont't need send  to the client

// Set read permissions for the 'companymember' collection for the 'Pro' role
const readPermission = new CActionPermission("read");
readPermission.allowed = true;
readPermission.immutableprops = ["_id", "company", "ownerapproval", "adminapproval", "title", "message", "draft", "deleted"]; // Define the properties that can't be updated by 'Pro' role
...
// Set update permissions for the 'companymember' collection for the 'Pro' role
const updatePermission = new CActionPermission("update");
updatePermission.allowed = true;
updatePermission.immutableprops = ["_id", "company", "member", "adminapproval", "title", "message", "draft", "deleted"]; // Define the properties that can't be updated by 'Pro' role
...
// Set delete permissions for the 'companymember' collection for the 'Pro' role
