...
// Set create permissions for the 'companymember' collection for the 'Admin' role
const createPermission = new CActionPermission("create");
createPermission.allowed = true;
createPermission.immutableprops = ["_id", "ownerapproval", "adminapproval", "draft", "deleted"]; // Define the properties that can't be updated by 'Admin' role
createPermission.hiddenprops = ["__v"]; // Define the properties dont't need send  to the client

// Set read permissions for the 'companymember' collection for the 'Admin' role
const readPermission = new CActionPermission("read");
readPermission.allowed = true;
readPermission.immutableprops = ["_id", "company", "member", "ownerapproval", "adminapproval", "draft", "deleted"]; // Define the properties that can't be updated by 'Admin' role
...
// Set update permissions for the 'companymember' collection for the 'Admin' role
const updatePermission = new CActionPermission("update");
updatePermission.allowed = true;
updatePermission.immutableprops = ["_id", "company", "member", "ownerapproval", "draft", "deleted"]; // Define the properties that can't be updated by 'Admin' role
...
// Set delete permissions for the 'companymember' collection for the 'Admin' role
