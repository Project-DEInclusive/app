...
import Schema from "../../../../types/schema/company/SCompany";
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "companies"); // Get the permissions for the current obj
...
async function handlePOST(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const user = await User.findById(obj.owner, { name: 1, email: 1, phone: 1, thumbnail: 1 });
        await Member.create({
            company: { id: resp._id, name: resp.name, thumbnail: resp.thumbnail, email: resp.email, website: resp.website, phone: resp.phone, owner: resp.owner },
            memberrole: EMemberRole.owner,
            member: { id: user._id, name: `${user.name.first} ${user.name.last}`, email: user.email, phone: user.phone, thumbnail: user.thumbnail },
            title: `Create Company '${resp.name}'.`,
            ownerapproval: EApprovalState.approved,
            adminapproval: EApprovalState.approved,
        });
