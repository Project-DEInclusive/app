...
import Schema from "../../../../types/schema/company/SCompanyMember";
...
async function handlePOST(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        console.log(obj);
        const company = await Company.findById(obj.company.id, { _id: 1, name: 1, owner: 1, thumbnail: 1, website: 1, phone: 1 }); // Get the company name
        if (!company) return res.status(404).json({ error: "Company not found" }); // Check if the company exists
        if (company.owner == userId || userType == EUsertype.admin || userType == EUsertype.superadmin) {
            if (!obj.member.id || obj.member.id.length == 0) return res.status(400).json({ error: "Member Id Undefined" });
        } else {
            obj.member.id = userId; // Set the member id
            obj.memberrole = EMemberRole.regular;
        }
        obj.company = {
            id: company._id,
            name: company.name,
        ...
        obj.ownerapproval = EApprovalState.pending; // Set the status to pending
        obj.adminapproval = EApprovalState.pending; // Set the status to pending
