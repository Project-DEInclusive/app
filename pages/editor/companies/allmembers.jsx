...
import fetcher from "../../../constants/fetch/companymember";

import useUserStore from "../../../constants/stores/userstore";

import dynamic from "next/dynamic";
const View = dynamic(() => import("./viewmember"));
const Add = dynamic(() => import("./addmember"));
...
const Datatable = (props) => {
    ...
    const selectActions = [
        {
            ...
            button: () => (
                ...
                >
                    Add Member <PlusIcon className="ml-2 h-5 w-5" aria-hidden="true" />
                </button>
            ),
        },
        {
    ...
    useEffect(() => {
        if (props.query?.search) setSearch(props.query?.search);
        if (!props.query?.page) props.query.page = 1;
        if (!props.query?.itemsperpage) props.query.itemsperpage = 10;
        if (!props.query?.sort) props.query.sort = JSON.stringify({ createdAt: -1 });
        props.push({ query: { ...props.query } });
        updateUI();
    }, []);
    ...
    const setPageSize = (pageSize) => {
        props.query.itemsperpage = pageSize;
        props.push({ query: { ...props.query } });
        updateUI();
    };
    // set sorting column
    ...
    const fetchdata = async () => {
        const res = await fetcher.get(
            ...
            true,
            false,
            props.query?.itemsperpage,
            props.query?.page - 1,
            props.query?.sort ? JSON.parse(props.query?.sort) : { createdAt: -1 },
            setLoading
        );
        if (res != null) {
            setSelectedData([]);
    ...
    const buildSearchQuery = () => {
        let query = { ...props.query?.filter, ...props.query?.query?.filter };
        if (search && search != "" && props.query?.searchby) {
            if (props.query?.searchby == "") {
                let tmpquery = {};
                ...
                for (let key in columns) {
                    if (columns[key].searchable) query[props.query?.searchby] = { $regex: search, $options: "i" };
                }
            }
        }
        return query;
    ...
    const getSortIcon = (column) => {
        if (!column.sortable || !props.query?.sort) return <></>;
        if (props.query?.sort.includes(column.selector) && props.query?.sort.includes("-"))
            return (
                <SortAscendingIcon
        ...
        else if (props.query?.sort.includes(column.selector))
            return (
                <SortDescendingIcon
                    className="h-5 w-5 text-gray-400 float-right"
                    aria-hidden="true"
    ...
    // update the data
    const updateRecordMember = async (item) => {
    ...
    // add the data
    const addRecordMember = async (item) => {
        const usr = await fetcher.create(item, false, () => {});
        if (usr) {
    ...
    const gotoPreviousPage = () => {
        if (props.query?.page > 1) {
            setPage(Number.parseInt(props.query?.page) - 1);
        }
    };
    // goto next page
    const gotoNextPage = () => {
        if (props.query?.page < pages) {
            setPage(Number.parseInt(props.query?.page) + 1);
        }
    };
    ...
    const updateStatus = async () => {
        ...
        const dta = await fetcher.updateProps(
            { _id: { $in: selectedData } },
            user.value.status >= EUsertype.admin ? { adminapproval: EApprovalState[updatingStatus] } : { ownerapproval: EApprovalState[updatingStatus] },
            false,
            setLoadingUpdate
        );
        if (dta) {
    ...
    return (
        <>
                    ...
                    <div className="flex flex-row justify-between mx-5 my-4 font-normal text-sm">
                        <div className="w-1/2 flex md:justify-start sm:justify-center text-gray-600">
                            <div>
                                ...
                                <select
                                    ...
                                    value={props.query?.itemsperpage}
                                >
                        ...
                        <div className="w-1/2 flex md:justify-end sm:justify-center text-gray-600">
                            <div className="mt-1 relative rounded-md shadow-sm w-full">
                                ...
                                <form
                                    onSubmit={(e) => {
                                        e.prcompanymemberDefault();
                                        if (search && search != "" && props.query?.search != search) {
                                ...
                                >
                                    ...
                                    />
                                    <button className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-companymembers-none" type="submit">
                    ...
                    <div className="flex flex-row justify-between mx-5 mt-0 my-4 font-normal text-sm">
                        <div className=" flex flex-wrap text-gray-600">
                            <div>
                                ...
                                {props.query?.search && props.query?.searchby && (
                                    <button
                                        type="button"
                                    ...
                                    data.map((item, index) => {
                                        return (
                                            <tr key={index} className="text-gray-600">
                                                ...
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{item.company.name}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{item.title}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{item.member.name}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{getRoleStatus(item.memberrole)}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{item.member.email}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{item.member.mobile}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{getDate(item.createdAt)}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{getStatus(item.ownerapproval)}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <div className="text-sm leading-5">{getStatus(item.adminapproval)}</div>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                ...
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200 text-right text-sm leading-5 font-medium">
                                                    <div className="flex items-center justify-center">
                                                        <button className="text-green-600 hover:text-green-900 focus:outline-none focus:shadow-outline" onClick={() => _view(item)}>
                    ...
                    <div className="flex flex-row justify-between mx-5 my-4 font-normal text-sm">
                        <div className="w-1/2 flex md:justify-start sm:justify-center text-gray-600">
                            <div className="rounded-md border border-gray-200 text-sm font-medium text-indigo-500 px-4 py-2">
                                Showing page {props.query?.page} of {pages}. {total} total records.
                            </div>
                        </div>
                        {pages > 0 && (
                                        ...
                                        .map((_, index) => (
                                            <button
                                                ...
                                                className={`rounded-full p-2 w-9 border border-gray-200  text-sm  ${
                                                    props.query.page == index + 1 ? "bg-indigo-500 text-white" : "bg-gray-100 text-gray-500"
                                                }`}
            ...
            <SidePanel title={`Company Member Information`} setOpen={setViewingOpen} isOpen={viewingOpen}>
                {viewingData && (
                    <View
                        readOnlyProps={[
                            "title",
            ...
            <SidePanel title={`Add Member to Company`} setOpen={setEditingOpen} isOpen={editingOpen}>
                <Add
                    data={editingData}
                    edit={true}
                    onSubmit={addRecordMember}
                    onCancel={() => {
                        setEditingOpen(false);
                    }}
                />
