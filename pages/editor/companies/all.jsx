...
import memberfetcher from "../../../constants/fetch/companymember";

import dynamic from "next/dynamic";
const Add = dynamic(() => import("./add"));
const AllMembers = dynamic(() => import("./allmembers"));
...
const Datatable = (props) => {
    ...
    const selectActions = [
        {
            ...
            button: () => (
                ...
                >
                    Add Company <PlusIcon className="ml-2 h-5 w-5" aria-hidden="true" />
                </button>
            ),
        },
        {
    ...
    const _memberview = async (item) => {
        setMemberData({ filter: { "company.id": item._id } });
        setMemberOpen(true);
    ...
    // get 'yes' or 'no' according to the boolean value
    ...
    return (
        <>
                                    ...
                                    data.map((item, index) => {
                                        return (
                                            <tr key={index} className="text-gray-600">
                                                ...
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                    <a href={item.website} className="text-sm leading-5 text-indigo-700 hover:text-indigo-500">
                                                        {item.website}
                                                    </a>
                                                </td>
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                                ...
                                                <td className="px-6 py-2 whitespace-no-wrap border-b border-gray-200 text-right text-sm leading-5 font-medium">
                                                    <div className="flex items-center justify-center">
                                                        <button
                                                            className="text-purple-600 hover:text-purple-900 focus:outline-none focus:shadow-outline"
                                                            onClick={() => _memberview(item)}
                                                        >
                                                            <UsersIcon className="h-5 w-5" aria-hidden="true" />
                                                        </button>
                                                        <button
                                                            className="text-indigo-600 hover:text-indigo-900 focus:outline-none focus:shadow-outline ml-4"
                                                            onClick={() => _edit(item)}
                                                        >
            ...
            <SidePanel title={`Edit Company '${editingData?._id}'`} setOpen={setEditingOpen} isOpen={editingOpen}>
                {editingData && (
                    <Add
                        data={editingData}
                        edit={true}
            ...
            <SidePanel title={`Company Users '${memberData?.query?.filter["company.id"]}'`} setOpen={setMemberOpen} isOpen={memberOpen}>
                {memberData && <AllMembers query={memberData} push={setMemberData} />}
            </SidePanel>
