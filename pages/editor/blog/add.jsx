...
const Add = (props) => {
    // form state object
    const [dataObj, setDataObj] = useState(props.data || new CBlog());
    ...
    const getCollections = async () => {
        if (_collections == null) _setCollections(await collectionfetcher.getCollections(["blogcategory"], () => {}));
    };
    ...
    const handleImage = async (e) => {
        try {
            ...
            const imgok = await filehandler.uploadBlogThumbnail(key, file);
            if (imgok) {
                ...
                handleChange({ target: { name: "thumbnail.src", value: `blogs/bp-${key}.png` } });
            } else {
                    ...
                    <div className="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
                        ...
                        <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                            ...
                            <div className="mt-1 sm:mt-0 sm:col-span-2">
                                ...
                                />
                                <p className="mt-2 text-sm text-gray-500">Write a few sentences about blog.</p>
                        ...
                        {editorLoaded && (
                            <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                ...
                                <div className="mt-1 sm:mt-0 sm:col-span-2">
                                    ...
                                    />
                                    <p className="mt-2 text-sm text-gray-500">Your blog article content.</p>
                            ...
                            <div className="mt-1 sm:mt-0 sm:col-span-2">
                                <div className="max-w-lg flex justify-center pb-6 rounded-md">
                                    {dataObj.thumbnail && dataObj.thumbnail.src && (
                                        <img
                                            src={`${process.env.NEXT_PUBLIC_IMAGE_URL}${dataObj._id || "blog"}${thumblstup}?key=${dataObj.thumbnail.src}`}
                                            alt="Blog banner"
                                            className="h-full w-full rounded-md"
                                        />
                                    )}
                                    {(!dataObj.thumbnail || !dataObj.thumbnail.src) && thumbnail && <img src={thumbnail} alt="Blog banner" className="h-full w-full rounded-md" />}
                                </div>
