...
const Index = ({ _collections }) => {
    ...
    const [newsposts, setNewsPosts] = useState([]); // newsposts object
    const [clearNewsPosts, setClearNewsPosts] = useState(false);
    ...
    const fetchdata = async () => {
        const res = await fetcher.get_Public(
            ...
            false,
            10,
            clearNewsPosts ? 0 : currentPage - 1,
            router.query.sort ? JSON.parse(router.query.sort) : { createdAt: -1 },
            setLoading
        );
        if (res != null) {
            if (clearNewsPosts) {
                setNewsPosts(res.values);
                setCurrentPage(1);
                setClearNewsPosts(false);
            } else setNewsPosts([...newsposts, ...res.values]);
            setTotal(res.count);
    ...
    return (
        <>
            <Head>
                <title>News posts - DEInclusive</title>
            </Head>
            <main className="max-w-7xl mx-auto">
                <div className="md:pt-6 lg:py-12">
                    <h1 className="text-gray-800 font-bold text-3xl text-center mb-4 md:mb-0 ">News</h1>
                    <div className="mx-auto justify-center px-4 py-t md:pt-6 lg:pt-12 sm:px-6 md:px-8 flex flex-col lg:flex-row gap-x-5">
                        <div className="mt-1 mb-2 flex rounded-md shadow-sm w-full lg:w-5/12">
                            ...
                            <input
                                ...
                                placeholder="Keyword, news title, or company name"
                    ...
                    <div className="mt-10 lg:mt-0">
                        ...
                        <div className="mt-4 flex flex-row ">
                            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 flex-wrap ">
                                {newsposts &&
                                    newsposts.length > 0 &&
                                    newsposts?.map((news, index) => <SingleBlogCard key={index} blog={news} onClick={() => router.push(`/news/${news._id}`)}></SingleBlogCard>)}

                                {(!newsposts || filterChange || newsposts.length == 0) &&
                                    loading &&
                                    Array(10)
                                        .fill(1)
                                        .map((_, index) => <SingleBlogCardLoading key={index}></SingleBlogCardLoading>)}
                                {newsposts && total > newsposts.length && (
                                    <button
                                        disabled={loading}
                                        onClick={loadMore}
